...
public class JpaDAOImpl<K, E> implements JPADAO<E, K> {
	...
	public List<E> find(final int startFrom, final int maxResults) {
		 ...
		 * try{ Query q = em.createQuery("SELECT h FROM " +
		 * entityClass.getName() + " h"); q.setMaxResults(maxResults);
		 * q.setFirstResult(startFrom); return q.getResultList(); }
		 * catch(PersistenceException e){ e.printStackTrace(); throw e; } }
		 * 
		 * }));
		 */
	...
	public List<E> findByNamedQueryAndNamedParams(final String name, final Map<String, ? extends Object> params) {
		...
		for (final Map.Entry<String, ? extends Object> param : params.entrySet()) {
			...
			//query.setParameter(companyName,"1234abc");
			//query.setParameter(companyrn,"1234abc");
			
		}

		//select * from company where companyName=?companyName and companyRegNo=?companyRegNo
